...
func (*SyscallAccess) SpecificationForAttribute(name string) elemental.AttributeSpecification {
	...
	if v, ok := SyscallAccessAttributesMap[name]; ok {
		return v
	}

	// We could not find it, so let's check on the lower case indexed spec map
	return SyscallAccessLowerCaseAttributesMap[name]
}
...
// SyscallAccessLowerCaseAttributesMap represents the map of attribute for SyscallAccess.
var SyscallAccessLowerCaseAttributesMap = map[string]elemental.AttributeSpecification{
	"pid": elemental.AttributeSpecification{
		AllowedChoices: []string{},
		Autogenerated:  true,
		Description:    `PID is the PID of the process that used the system call.`,
