...
##### `pingType` `enum(None | AporetoIdentity | CustomIdentity | AporetoIdentityPassthrough)`
...
##### `type` `enum(Boolean | Duration | Enum | IntegerSlice | Integer | Float | FloatSlice | Password | String | StringSlice | CVSSThreshold | JSON | TagsExpression)` [`required`]
