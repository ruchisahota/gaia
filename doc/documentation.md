...
##### `realm` `emum(AWSSecurityToken | Certificate | Google | LDAP | Vince | GCPIdentityToken | AzureIdentityToken | OIDC | SAML | PCC | PCCIdentityToken)` [`required`]
...
Retrieves the list of the OIDC providers.
...
Allows to declare a PCC auth provider that can be use to trust PCC JWT.
...
##### `platform` `string` [`autogenerated`,`read_only`]
...
##### `remotePlatform` `string` [`autogenerated`,`read_only`]
